...
void MFRC522_I2C::PCD_Init() {
	...
	if (digitalRead(_resetPowerDownPin) == LOW) {	//The MFRC522 chip is in power down mode.
		...
		delay(100);
		Serial.println("Perfromed a hard reset");
	}
	else { // Perform a soft reset
		PCD_Reset();
		Serial.println("performed a soft reset");
	}
...
bool MFRC522_I2C::PCD_PerformSelfTest() {
	...
	switch (version) {
		...
		case 0x92:	// Version 2.0
			...
			break;
		case 0xB2:
			reference = MFRC522_JSMSEMI_firmware_reference;
			break;
 ...
 */
byte MFRC522_I2C::PCD_getAddress(){
	return _chipAddress;
} //End PCD_getAddress()
...
byte MFRC522_I2C::MIFARE_Ultralight_Write(	byte page, 		///< The page (2-15) to write to.
	...
	if (page < 4 || page > 39) { //Shouldn't write to those bits -> for now at least
		return STATUS_INVALID;
	}
...
void MFRC522_I2C::PICC_DumpMifareUltralightToSerial() {
	...
	// Try the mpages of the original Ultralight. Ultralight C has more pages. NTAG213 has 45 pages
	// User memory is located in the pages 4 - 39. See datasheet for more info.
	for (byte page = 0; page < 44; page +=4) { // Read returns data for 4 pages at a time. We don't read the last byte because of reasons
		// Read pages
		byteCount = sizeof(buffer);
...
/**
 * Dumps memory contents of a MIFARE Ultralight PICC To a buffer
 * ultralightData is the pntr to the start of a 176 byte buffer
 */
void MFRC522_I2C::PICC_DumpMifareUltralightToBuffer(byte* ultralightData) {
	byte status;
	byte byteCount;
	byte buffer[18];
	byte i;
	...
	for (byte page = 0; page < 44; page +=4) { // Read returns data for 4 pages at a time.
		// Read pages
		byteCount = sizeof(buffer);
		status = MIFARE_Read(page, buffer, &byteCount);
		if (status != STATUS_OK) {
		...
		for (byte offset = 0; offset < 4; offset++) {
			for (byte index = 0; index < 4; index++) {
				i = 4 * offset + index;
				j = i+page*4;
				ultralightData[j] = buffer[i];
			}
		}
	}
} // End PICC_DumpMifareUltralightToEthernet()
